...
type ProductStore = {
  ...
  getProduct: (id: string) => Promise<Product<Variant> | null>;
...
export const useProductStore = create<ProductStore>((set, get) => ({
      ...
      set({
        ...
        error: {
          message:
            axiosError.response?.data?.message ||
            axiosError.message ||
            "Failed to getProductCategories",
          details: axiosError.response?.data?.details,
        },
      });
    }
  ...
  getProductsForClient: async (params: { page?: number; limit?: number }) => {
    try {
      set({ isLoading: true, error: null });
      const response = await axiosClient.get("/product/get-products", {
        params,
      ...
      set({
        ...
        error: {
          message:
            axiosError.response?.data?.message ||
            axiosError.message ||
            "Failed to get products for client",
          details: axiosError.response?.data?.details,
        },
      });
    }
