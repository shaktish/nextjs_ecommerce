...
const updateProduct = asyncHandler(
  async (req: AuthenticateRequest, res: Response) => {
    ...
    try {
      ...
      const sizeIds = (value.variants || []).map(
        (v: CreateVariantDTO) => v.sizeId,
      );
...
const getProductsForClient = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const scopeCategories = await prisma.category.findMany({
      where: {
        OR: [{ id: { in: categories } }, { parentId: parent?.id }],
      },
      select: { id: true },
    });
    ...
    const sizes = value.sizes ? value.sizes.split(",").filter(Boolean) : [];

    const brands = value.brands ? value.brands.split(",").filter(Boolean) : [];
