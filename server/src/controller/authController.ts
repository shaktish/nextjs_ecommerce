...
import { compassPassword, hashPassword } from "../utils/hashPassword";
import { generateTokens } from "../utils/jwtHelper";
import { clearTokens, setTokens } from "../utils/authHelper";
...
const login = asyncHandler(
  async (req: Request<{}, {}, LoginRequestBody>, res: Response) => {
    ...
    const isPasswordValid = await compassPassword(
      password,
      user?.password ?? "",
    );
...
const refreshAccessToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .json({ message: "Access token updated Successfully", userInfo });
});
